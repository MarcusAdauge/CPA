...
// *entrada = "~/UPV/CPA/Prac2/binLenna1024c.ppm",
  while (*++argv) {
    ...
    case 'h':
    default:
      fprintf(stderr, "Usage: encaja-e2-px -i Input.ppm -o Output.ppm -t -h\n");
      return 1;
    }
  }
  ...
  printf("Parallel image reconstruction time (loop: x): %f\n", t2-t1);
...
// /labos/asignaturas/ETSINF/cpa/p2